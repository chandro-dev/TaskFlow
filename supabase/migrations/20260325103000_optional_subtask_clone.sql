create or replace function public.clone_project_task(
  ...
  target_subtasks jsonb default null,
...
begin
  ...
  -- Null means "use the full source checklist". An explicit empty array means
  -- "clone without subtasks", which keeps UI intent intact.
  if target_subtasks is null then
  ...
  elsif coalesce(jsonb_array_length(target_subtasks), 0) > 0 then
    insert into public.task_subtasks (task_id, title, is_completed)
    select
      created_task.id,
